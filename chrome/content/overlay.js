var titlecasehistory = new SimpleUndo({
	maxLength: 200,
	provider: function(done) {
		done(document.commandDispatcher.focusedWindow.document.documentElement.innerHTML);
	},
	onUpdate: function() {
		//onUpdate is called in constructor, making history undefined
		if (!titlecasehistory) return; 
...
var titlecasethunderbird = {
  onLoad: function() {
	...
	document.addEventListener("keydown", function(e){
		var ctrlkeypress = e.ctrlKey; 	// CTRL
		var altkeypress = e.altKey; 	// ALT
		var ykeypress = 89;	// y
		var ykeypress2 = 121;	// Y
		var zkeypress = 90;	// z
		var zkeypress2 = 122;	// Z
		
		if (prefs.getBoolPref("ctrlz")) {
			if((e.which == zkeypress || e.which == zkeypress2) && ctrlkeypress == true) {
					titlecasethunderbird.undo(e);
					return false;
			}
			if((e.which == ykeypress || e.which == ykeypress2) && ctrlkeypress == true) {
					titlecasethunderbird.redo(e);
					return false;
			}
		} else {
			if((e.which == zkeypress || e.which == zkeypress2) && altkeypress == true && ctrlkeypress == true) {
					titlecasethunderbird.undo(e);
					return false;
			}
			if((e.which == ykeypress || e.which == ykeypress2) && altkeypress == true && ctrlkeypress == true) {
					titlecasethunderbird.redo(e);
					return false;
			}
		}
		clearTimeout(timeSaver);
		timeSaver = setTimeout(function() {titlecasehistory.save();},300);
	});
  ...
  ie_has_no_indexOf: function(input) {
	...
	undo: function(event) {
		titlecasehistory.undo(titlecasethunderbird.setBody);
	},
	
	redo: function(event) {
		titlecasehistory.redo(titlecasethunderbird.setBody);
	},
  
	setBody: function(contents) {
		if (!contents) return false;
		document.commandDispatcher.focusedWindow.document.documentElement.innerHTML = contents;
	},
